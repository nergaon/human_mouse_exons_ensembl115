...
def select_best(group):
    ...
    return filtered.loc[filtered['star_count'].idxmin()]
    ...
    for i in range(0, len(mouse_ortologs)):
        ...
        try:
           ...
           if orthologs_df['position_h'].duplicated().any() or orthologs_df['position_m'].duplicated().any():
               orthologs_df = orthologs_df.groupby('position_h', group_keys=False).apply(select_best).reset_index(drop=True)
               orthologs_df = orthologs_df.groupby('position_m', group_keys=False).apply(select_best).reset_index(drop=True)
