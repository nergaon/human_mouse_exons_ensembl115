...
# Enforce minimum threshold of 2
  if [ "$threshold" -lt 2 ]; then
	...
	$leafcutter_scripts/leafcutter/scripts/leafcutter_ds.R --num_threads 4 -i "$threshold"  -g "$threshold" --exon_file=/gpfs0/tals/projects/data/Transcriptomes/human_hg38/GCF_000001405.39_GRCh38.p13_genomic_leafcutter.txt.gz AS_clusters_value_HN4.txt groups_file.txt
done
