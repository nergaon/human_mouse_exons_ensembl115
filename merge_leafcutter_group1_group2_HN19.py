...
def get_rows(points_df, df, col_name):
    ...
    genomic_col_start = "start_" + species_dict[species]
    genomic_col_end = "end_" + species_dict[species]
    ...
    rows = []
    junction_dict = {}
    row = None  # Initialize row to return
    #run_index=0
    for i in df.index:
        # if run_index%1000 == 0:
        #     print(run_index)
        ...
        row = df.loc[[i]].copy()
        row[genomic_col_start] = value_a
        row[genomic_col_end] = value_b
        row[junction_col] = f"{chrName}:{junction_dict[point_a]}:{junction_dict[point_b]}"
        rows.append(row)
    # Combine all rows into a single DataFrame
    orthologs_group = pd.concat(rows)
    orthologs_group.drop([genomic_col_start, genomic_col_end], axis=1, inplace=True)
    # Modify index format
...
def main():
    ...
    points_input = ensembl_dir + 'unique_points.txt' #all the ortholgs points
