...
def filter_group(group):
    max_id = group['Average %id'].max()
    filtered = group[group['Average %id'] == max_id]
    min_star = filtered['star_count'].min()
    final = filtered[filtered['star_count'] == min_star]
    return final.head(1)  # take only one row arbitrarily if still tied
  ...
  #/gpfs0/tals/projects/Analysis/human_mouse_exons/scripts/ensembl113->qsub -cwd -V -q tals.q run_transcripts_exons_all_HN3.sh
    ...
    if num_exons_h > 1 and num_exons_m > 1: #at least 2 exons
        ...
        if len(junctions)>2: #more than 1 ortholog exons
            ...
            # Get the max identity and minimum '*' count per group in cases the mouse postion and the human position are identical
            df_filtered = junctions.groupby(['position_h', 'position_m'], group_keys=False).apply(filter_group)
