...
def get_ensembl_transcripts_exons(ensembl):
    ...
    if "Transcript" in gene_data:
        ...
        sorted_transcripts = sorted(
            transcript_info,
            key=lambda t: (
                0 if t.get("is_canonical") else
                1 if t.get("biotype") == "protein_coding" else
                2,
                -transcript_length(t)  # longer transcripts come first
            )
        )
...
#function to calculate transcript length
def transcript_length(t):
    return (t.get("end", 0) or 0) - (t.get("start", 0) or 0)
...
def alignment(exons_seq_h, exons_seq_m, dir):
    ...
    #exons_names = list(all_exons.keys())
    ...
    for x in range(0, len(concatenated_dict)):
        ...
        for y in range(0, len(concatenated_dict)):
            ...
            if len(name_1_seq) > 0 and len(name_2_seq) > 0:
                ...
                df_score[name_1][name_2] = alignments.score
            else: #exons that are only UTR
                percentage_identity = -100
                df_score[name_1][name_2] = -100
            df_id[name_1][name_2] = percentage_identity
...
def get_rank(canonical_exons, exon):
    rank = get_exon_position(exon, canonical_exons)
    return(rank)

def get_exon_position(exon_id, canonical_exons):
    return canonical_exons.get(exon_id, "*")
...
# Function to count '*' in both rank_h and rank_m
def count_stars(group):
    return (group["rank_h"] == "*").sum() + (group["rank_m"] == "*").sum()
...
def main():
  #/gpfs0/tals/projects/Analysis/human_mouse_exons/scripts/ensembl113->qsub -cwd -V -q tals.q run_transcripts_exons_all_HN3.sh
    ...
    columns_junctions = ['ensembl', 'symbol', 'exon_h', 'rank_h', 'position_h', 'exon_m', 'rank_m', 'position_m']
    columns_statistics = ['ensembl','gene','transcripts','exons','unique_positions','exons_with_ortholog','exons_with_no_orthologs','E1','E2']
    ...
    new_dir = working_dir + 'genes/' + ensembl_h + "_" + str(i)
    # Create the directory and its parents if they don't exist
    os.makedirs(new_dir, exist_ok=True)
    ortholgs_output = new_dir + "/orthologs_points.txt"
    ortholgs_unique_output = new_dir + "/orthologs_unique_points.txt"
        ...
        for exon_h_from_m_df in mouse_id_filter.loc[exon_m, 'max_column']:
            for exon_m_from_h_df in human_id.loc[exon_h_from_m_df, 'max_column']:
                ...
                if exon_m_from_h_df == exon_m and human_id_mouse >= min_id:
                    ...
                    new_row = pd.DataFrame([[ensembl_h, gene_h, exon_h_from_m_df, exons_rank_h, start_h, exon_m_from_h_df, exons_rank_m, start_m]], columns=junctions.columns)
                    junctions = pd.concat([junctions, new_row], ignore_index=True)
                    new_row = new_row = pd.DataFrame([[ensembl_h, gene_h, exon_h_from_m_df, exons_rank_h, end_h, exon_m_from_h_df, exons_rank_m, end_m]], columns=junctions.columns)
                    junctions = pd.concat([junctions, new_row], ignore_index=True)
    if len(junctions)>2: #more than 1 ortholog exons
        junctions.to_csv(ortholgs_output, sep="\t") 
        #get unique junctions
        # Compute star count per (position_h, position_m) group
        star_counts = junctions.groupby(["exon_h", "position_h", "exon_m", "position_m"]).apply(count_stars)
        # Merge the computed star counts back into the DataFrame
        junctions = junctions.merge(star_counts.rename("star_count"), on=["exon_h", "position_h", "exon_m", "position_m"])
        # Get the minimum '*' count per group in cases the mouse postion and the human position are identical
        min_star_counts = junctions.groupby(["position_h", "position_m"])["star_count"].transform("min")
        # Filter rows where the '*' count is the minimum for that group
        df_filtered = junctions[junctions["star_count"] == min_star_counts].drop(columns=["star_count"])
        # Keep only the first occurrence per (position_h, position_m)
        df_filtered = df_filtered.drop_duplicates(subset=["position_h", "position_m"], keep="first")
        df_filtered.to_csv(ortholgs_unique_output, sep="\t")
    
         #all the exons with min_id 
        good_orthologs_h = (human_id['max_value'] >= min_id).sum()
        good_orthologs_m = (mouse_id['max_value'] >= min_id).sum()
        num_unique_positions_h = df_filtered["position_h"].nunique()
        num_unique_positions_m = df_filtered["position_m"].nunique()
        #columns_statistics = ['ensembl','gene','transcripts','exons','unique_positions','exons_with_ortholog','exons_with_no_orthologs','E1','E2']
        data_df_h.loc[ensembl_h] = [gene_h,len(tra_exons_h),len(all_exons_h),num_unique_positions_h,good_orthologs_h,(human_id['max_value']<min_id).sum(),exon_count_h['E1'],exon_count_h['E2']]
        data_df_m.loc[ensembl_m] = [gene_m,len(tra_exons_m),len(all_exons_m),num_unique_positions_m,good_orthologs_m,(mouse_id['max_value']<min_id).sum(),exon_count_m['E1'],exon_count_m['E2']]
        ...
        output = working_dir + "genesWithOneOrthologExon.txt"
